...
create_conoha_dns_record(){
  curl -sS https://dns-service.tyo2.conoha.io/v1/domains/${CNH_DOMAIN_ID}/records \
  -X POST \
  -H "Accept: application/json" \
  -H "Content-Type: application/json" \
  -H "X-Auth-Token: ${CNH_TOKEN}" \
...
delete_conoha_dns_record(){
  local delete_id=$1
  curl -sS https://dns-service.tyo2.conoha.io/v1/domains/${CNH_DOMAIN_ID}/records/${delete_id} \
  -X DELETE \
  -H "Accept: application/json" \
  -H "Content-Type: application/json" \
  -H "X-Auth-Token: ${CNH_TOKEN}"
