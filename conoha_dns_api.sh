...
# -------- #
get_conoha_token(){
  curl -sS https://identity.${REGION}.conoha.io/v2.0/tokens \
  -X POST \
...
get_conoha_domain_id(){
  curl -sS https://dns-service.${REGION}.conoha.io/v1/domains \
  -X GET \
  -H "Accept: application/json" \
...
create_conoha_dns_record(){
  curl -sS https://dns-service.${REGION}.conoha.io/v1/domains/${CNH_DOMAIN_ID}/records \
  -X POST \
  -H "Accept: application/json" \
  -H "Content-Type: application/json" \
  -H "X-Auth-Token: ${CNH_TOKEN}" \
...
delete_conoha_dns_record(){
  local delete_id=$1
  curl -sS https://dns-service.${REGION}.conoha.io/v1/domains/${CNH_DOMAIN_ID}/records/${delete_id} \
  -X DELETE \
  -H "Accept: application/json" \
  -H "Content-Type: application/json" \
  -H "X-Auth-Token: ${CNH_TOKEN}"
